   ...
   return (
      <section className="bg-[#fcfbf9] border-t border-black/5 max-w-none w-full py-16 md:py-28 px-6">
         <div className="max-w-4xl mx-auto">
            <h2 className="font-serif mb-12 md:mb-24 text-text-primary text-[32px] leading-[1.15] font-medium md:text-5xl lg:text-6xl flex flex-nowrap justify-center items-center gap-2 md:gap-5 text-center px-2 w-full">
            ...
            <div className="relative border-l border-[#c0d6c0] ml-6.25 md:ml-[50%] space-y-16">
               {schedule.map((item, idx) => (
                  <div key={idx} className={`relative flex flex-col md:flex-row md:justify-between items-center md:-ml-[50%] w-full ${idx % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
                     {/* Dot */}
                     <div className="absolute -left-7.25 md:left-[50%] md:-ml-[6.5px] w-3 h-3 bg-[#a2c2a2] rounded-full z-10 shadow-[0_0_0_6px_#f6e5db]" />

                     <div className={`w-full md:w-5/12 ml-8 md:ml-0 ${idx % 2 === 0 ? 'md:text-right pr-0 md:pr-12' : 'md:text-left pl-0 md:pl-12'}`}>
                        <h3 className="text-[18px] leading-[1.3] font-medium md:text-3xl font-serif text-accent mb-2">{item.time}</h3>
                        <h4 className="font-sans text-[22px] leading-tight font-medium md:text-2xl mb-2 text-text-primary tracking-wide">{item.title}</h4>
                        <p className="font-sans text-[16px] leading-normal font-normal text-text-secondary md:text-lg">{item.desc}</p>
                     </div>
